  ...
  class Body < LacunaModule
    ...
    def resource_rates
      ...
      rates.ores.members.each do |ore_type|
        value = @status[:ore][ore_type].to_i || 0
    ...
    def resources_available
      ...
      buildings.each do |building|
        case building.building_module_name
        when /OreStorage/ then
          building.view[:ore_stored].each_pair do |type, amount|
            stored.ores[type] += amount.to_i
          end
        when /FoodReserve/ then
          building.view[:food_stored].each_pair do |type, amount|
            stored.foods[type] += amount.to_i
          end
        end
      end
    ...
    def buildings
      api_call('get_buildings')[:buildings].to_a.map do |i|
        Building.new(@session, i[0].to_s.to_i, i[1])
      end
    end
  end
end
